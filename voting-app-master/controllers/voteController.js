...
exports.generateVotingLink = async (req, res, next) => {
  ...
  try {
    ...
    if (existingVote) {
      return next(
        new ErrorResponse("You have already voted in this election", 400)
      );
    }
    ...
    const votingLink = `${req.protocol}://${req.get(
      "host"
    )}/api/v1/votes/cast/${votingToken}`;
...
exports.castVote = async (req, res, next) => {
  ...
  try {
    ...
    if (userId !== req.user.id) {
      return next(
        new ErrorResponse("Unauthorized to use this voting link", 401)
      );
    }
    ...
    if (!candidate) {
      return next(
        new ErrorResponse("Candidate not found in this election", 404)
      );
    }
    ...
    // if (faceImage) {
    //   const profile = await VoterProfile.findOne({ user: req.user.id }).select(
    //     "+faceData"
    //   );
